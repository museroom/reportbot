...
class MonthlyReportDetailView(LoginRequiredMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['add_photo_url'] = reverse('photologue:photo-select-popup-list',
                                           kwargs={'year': date_time.year, 'month': date_time.month,
                                                   'day': date_time.day,
...
class DailyReportListView(DailyReportDateView, ArchiveIndexView):
    ...
    def get_queryset(self):
        ...
        d = Photo
        qset = d.objects.filter(daily_report_item__daily_report__title=date_and_time).order_by(
            ...
            if res:
                ...
                if db_field not in non_db_field:
                    ...
                    if type(q_db_field) == timezone.datetime:
                        ...
                        cell.alignment = openpyxl.styles.alignment.Alignment(wrapText=True, vertical='top')
                        db_value = q_db_field
                    else:
                        db_value = q_db_field
                else:
